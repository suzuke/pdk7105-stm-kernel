...
EXPORT_SYMBOL(stm_amba_bridge_init);
...
static const struct {
	...
	{ type1_bitfields, ARRAY_SIZE(type1_bitfields), type1_reg_names},
	{ type2_bitfields, ARRAY_SIZE(type2_bitfields), type2_reg_names},
};

static int stm_amba_bridge_reg_show(struct seq_file *s, void *v)
{
	struct stm_amba_bridge *plug = s->private;
	...
	if (type == stm_amba_type2 && plug->config->type2.sd_config_missing) {
		a += 4;
		num_entries -= 4;
	}
...
static int stm_amba_bridge_reg_open(struct inode *inode, struct file *file)
{
	return single_open(file, stm_amba_bridge_reg_show, inode->i_private);
}

static const struct file_operations stm_amba_bridge_reg_read_fops = {
	.open = stm_amba_bridge_reg_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

#define STM_AMBA_BRIDGE_DEBUGFS_FOPS (&stm_amba_bridge_reg_read_fops)
#else
#define STM_AMBA_BRIDGE_DEBUGFS_FOPS NULL
#endif /* CONFIG_DEBUG_FS */

/* Number of times stm_amba_bridge_create called */
static atomic_t ref_count = ATOMIC_INIT(0);
static DEFINE_SPINLOCK(dfs_lock);
static struct dentry *dfs_dir;

static void stm_amba_bridge_devres_release(struct device *dev, void *res)
{
	struct stm_amba_bridge *plug = res;
