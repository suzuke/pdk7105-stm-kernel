...
#define STM_FDMA_MCHI_MAX_PAYLOAD	4096
#define STM_FDMA_MCHI_MIN_PACKET	3
#define STM_FDMA_MCHI_MAX_PACKET	(3 + 4096)
...
struct stm_fdma_mchi {
	struct stm_dma_dreq_config *pkt_start_rx_dreq;
...
{
	struct stm_dma_mchi_config *config = fchan->dma_chan.private;
	struct stm_fdma_mchi *mchi;
	int result;

	dev_dbg(fchan->fdev->dev, "%s(fchan=%p)\n", __func__, fchan);

	BUG_ON(fchan->type != STM_DMA_TYPE_MCHI);

	/* Allocate the mchi channel structure */
	mchi = kzalloc(sizeof(struct stm_fdma_mchi), GFP_KERNEL);
	if (!mchi) {
	...
	mchi->pkt_start_rx_dreq = stm_fdma_dreq_alloc(fchan,
	...
	result = stm_fdma_dreq_config(fchan, mchi->pkt_start_rx_dreq);
	if (result) {
...
error_rx_dreq_config:
	stm_fdma_dreq_free(fchan, mchi->pkt_start_rx_dreq);
error_rx_dreq_alloc:
error_dreq_config:
...
{
	struct stm_fdma_mchi *mchi = fchan->extension;
		...
		struct scatterlist *sgl, unsigned int sg_len)
{
	...
	for_each_sg(sgl, sg, sg_len, i)
		if (sg_dma_len(sg) < STM_FDMA_MCHI_MAX_PACKET) {
			dev_err(fchan->fdev->dev, "sgl[%d] length is less than"
					" max packet size (%d)\n", i,
					STM_FDMA_MCHI_MAX_PACKET);
			return NULL;
		}
