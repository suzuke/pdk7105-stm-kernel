...
static int create_controls(struct dentry *debugfs, void *c)
{
	...
	dropped_control = debugfs_create_file("dropped", 0, debugfs, NULL,
					      &dropped_fops);
	if (!dropped_control) {
		printk(KERN_ERR
	     ...
	     size_t count, loff_t *ppos)
{
	...
	if (kstrtol(buf, 10, &create) == -EINVAL)
		goto _fail;

	mtt_printk(KERN_DEBUG "create_write create=%ld\n", create);

	/* FIXME remove this property,
	 * we create the channel at init
	 */
	if (create && !my_chan) {
		my_chan = create_channel(subbuf_size, n_subbufs);
		if (!my_chan) {
			ret = -ENOSYS;
			goto _fail;
		}
	}
	/*else...do nothing, because some kprobes may still be armed,
	 * and will need the relay channel to exist.
	 */

	return count;

_fail:
	mtt_printk(KERN_ERR "create_write failed.\n");
	return -EINVAL;
}
...
mtt_drv_relay_config(struct mtt_sys_kconfig *cfg)
{
	...
	if (mtt_cur_out_drv == &relay_output_driver) {
		...
		/* Re/create channel, if not currently in use. */
		destroy_channel();
