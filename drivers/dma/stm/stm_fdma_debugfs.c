...
static char *stm_fdma_debugfs_fw_state[] = {
	"INIT", "LOADING", "LOADED", "ERROR"};
static char *stm_fdma_debugfs_fchan_state[] = {
	"IDLE", "RUNNING", "STOPPING", "PAUSING", "PAUSED", "ERROR"};
static char *stm_fdma_debugfs_direction[] = {
...
static int stm_fdma_debugfs_regs_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "--- %s (0x%p) (FW: %s) ---\n", dev_name(fdev->dev),
		   fdev->io_base, stm_fdma_debugfs_fw_state[fdev->fw_state]);
...
static int stm_fdma_debugfs_dmem_show(struct seq_file *m, void *v)
{
	struct stm_fdma_device *fdev = m->private;
	struct stm_plat_fdma_ram *dmem = &fdev->hw->dmem;
	int i, j;
...
static int stm_fdma_debugfs_dmem_open(struct inode *inode, struct file *file)
{
	return single_open(file, stm_fdma_debugfs_dmem_show, inode->i_private);
}

static const struct file_operations stm_fdma_debugfs_dmem_fops = {
	.open		= stm_fdma_debugfs_dmem_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static int stm_fdma_debugfs_chan_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "--- %s (0x%p) channel %d (%s) ---\n",
			...
			fchan->id, stm_fdma_debugfs_fchan_state[fchan->state]);
	...
	case STM_DMA_TYPE_TELSS:
		...
		for (i = 0; i < STM_FDMA_LLU_TELSS_HANDSETS; ++i) {
	...
	if (fchan->dreq) {
		...
		seq_printf(m, "%-30s              = %s\n", buffer,
			stm_fdma_debugfs_direction[fchan->dreq->direction]);
	}
