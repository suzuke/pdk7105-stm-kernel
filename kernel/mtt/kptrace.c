...
int __init mtt_kptrace_comp_alloc(void)
{
	...
	if (!err) {
		...
		mtt_comp_ctxt[0]->filter = MTT_LEVEL_NONE;
		return 0;
	}
...
void kptrace_mark(void)
{
	...
	data = mtt_kptrace_pkt_get(MTT_TRACEITEM_PTR32, &pkt, KEY_KM);
	*data = KEY_KM;
	mtt_kptrace_pkt_put(pkt);
}
...
static int irq_##EVENT_KEY##_handler(struct kretprobe_instance *ri, \
			       struct pt_regs *regs)\
{\
	long core;\
	mtt_packet_t *pkt;\
