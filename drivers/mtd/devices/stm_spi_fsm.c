...
static int s25fl_config(struct stm_spi_fsm *fsm, struct flash_info *info)
{
	...
	data_pads = ((fsm_seq_read.seq_cfg >> 16) & 0x3) + 1;
	fsm_read_status(fsm, FLASH_CMD_RDSR2, &cr1);
	if (data_pads == 4) {
		if (!(cr1 & S25FL_CONFIG_QE)) {
			/* Set 'QE' */
			cr1 |= S25FL_CONFIG_QE;

			fsm_read_status(fsm, FLASH_CMD_RDSR, &sr1);
...
static int __devinit stm_spi_fsm_probe(struct platform_device *pdev)
{
	...
	if (fsm->capabilities.boot_from_spi &&
	    !can_handle_soc_reset(fsm, info))
		dev_warn(&pdev->dev, "WARNING: no provision for SPI reset on boot-from-spi system\n");
	...
	if ((fsm->mtd.size > 16 * 1024 * 1024)  &&
		...
		dev_info(&pdev->dev, "reducing visibility to 16MiB (32-bit address mode not supported on device)\n");
