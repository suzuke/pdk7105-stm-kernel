...
{
	static __devinitdata char *fdma_clks_n[] = {
		"fdma_slim_clk",
		"fdma_hi_clk",
		"fdma_low_clk",
		"fdma_ic_clk",
	...
	pm_clk_init(&pdev->dev);
	for (i = 0; i < ARRAY_SIZE(fdma_clks_n); ++i) {
		struct clk *clk = clk_get(&pdev->dev, fdma_clks_n[i]);
		if (IS_ERR(clk))
			continue;
		clk_enable(clk);
		pm_clk_add(&pdev->dev, fdma_clks_n[i]);
	}
