...
#define STM_FDMA_MIN_BUFFER_SIZE	4

struct stm_fdma_audio {
	void *park_buffer;
...
static int stm_fdma_audio_alloc_parking_buffer(struct stm_fdma_chan *fchan,
		int buffer_size)
{
	struct stm_fdma_audio *audio = fchan->extension;
...
static void stm_fdma_audio_free_parking_buffer(struct stm_fdma_chan *fchan)
{
	struct stm_fdma_audio *audio = fchan->extension;
...
static int stm_fdma_audio_config(struct stm_fdma_chan *fchan,
		struct stm_dma_park_config *config)
{
	...
	if (config->buffer_size < STM_FDMA_MIN_BUFFER_SIZE) {
	...
	if (config->buffer_size > audio->park_buffer_size)
		stm_fdma_audio_free_parking_buffer(fchan);

	/* Check if parking buffer needs to be (re)allocated */
	if (!audio->park_buffer)
		stm_fdma_audio_alloc_parking_buffer(fchan, config->buffer_size);
...
{
	struct stm_dma_audio_config *config = fchan->dma_chan.private;
	struct stm_fdma_audio *audio;
	int result;

	dev_dbg(fchan->fdev->dev, "%s(fchan=%p)\n", __func__, fchan);

	BUG_ON(fchan->type != STM_DMA_TYPE_AUDIO);
	...
	audio = kzalloc(sizeof(struct stm_fdma_audio), GFP_KERNEL);
	if (!audio) {
	...
	fchan->desc_park = stm_fdma_desc_alloc(fchan);
	if (!fchan->desc_park) {
	...
	result = stm_fdma_audio_config(fchan, &config->park_config);
	if (result) {
...
error_audio_config:
	stm_fdma_desc_free(fchan->desc_park);
		...
		struct stm_dma_park_config *config)
{
	...
	/* Configure the descriptor (allocate dma buffer etc) */
	result = stm_fdma_audio_config(fchan, config);
...
int dma_audio_parking_enable(struct dma_chan *chan)
{
	...
	if (fchan->state != STM_FDMA_STATE_RUNNING) {
	...
	set_bit(STM_FDMA_IS_PARKED, &fchan->flags);
	stm_fdma_hw_channel_switch(fchan, fdesc, fchan->desc_park, 1);
...
int dma_audio_is_parking_active(struct dma_chan *chan)
{
	...
	return test_bit(STM_FDMA_IS_PARKED, &fchan->flags);
}
