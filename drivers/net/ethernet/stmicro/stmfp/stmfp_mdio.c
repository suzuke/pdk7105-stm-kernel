			   ...
			   u16 phydata)
{
	...
	fpdbg2("fpif_mdio_write phyaddr=%x phyreg=%x\n", phyaddr, phyreg);
	...
	fpdbg2("fpif_mdio_write phyaddr=%x phyreg=%x data=%x\n", phyaddr,
	       phyreg, phydata);
	fpdbg2("value in MDIO_CTL1 is %x\n", value);
	writel(value, priv->fpgrp->base + FP_MDIO_CTL1);
	/* Wait until any existing MII operation is complete */
	ret = fpif_wait_till_done(priv);
	fpdbg("WR:data val phyreg=%x read= %x\n", phyreg, phydata);
	return ret;
}
...
int fpif_mdio_register(struct net_device *ndev)
{
	...
	new_bus->parent = priv->devptr;
	fpdbg("mdiobus_register\n");
	...
	for (addr = 0; addr < PHY_MAX_ADDR; addr++) {
		...
		if (phydev) {
			...
			*/
			...
			fpdbg("%s: PHY ID %08x at %d IRQ %s (%s)\n",
			      ndev->name, phydev->phy_id, addr, irq_str,
			      dev_name(&phydev->dev));
			found = 1;
		}
	}
...
{
	struct fpif_priv *priv = netdev_priv(ndev);
	fpdbg("fpif_mdio_unregister\n");
	mdiobus_unregister(priv->mii);
	priv->mii->priv = NULL;
	mdiobus_free(priv->mii);
	priv->mii = NULL;
	fpdbg("fpif_mdio_unregister ends\n");
	return 0;
}
