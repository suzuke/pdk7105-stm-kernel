...
static u8 stm_miphy_dummy_reg_read(struct stm_miphy *miphy, u8 addr)
{
	return 0;
}

static void stm_miphy_dummy_reg_write(struct stm_miphy *miphy, u8 addr, u8 data)
{
}
...
static int stm_miphy_dummy_probe(struct platform_device *pdev)
{
	...
	int result;
	struct stm_miphy_device *miphy_dummy_dev;

	miphy_dummy_dev = devm_kzalloc(&pdev->dev,
				sizeof(*miphy_dummy_dev), GFP_KERNEL);
	if (!miphy_dummy_dev)
		return -ENOMEM;
	...
	platform_set_drvdata(pdev, miphy_dummy_dev);
...
static int stm_miphy_dummy_remove(struct platform_device *pdev)
{
	...
	miphy_dummy_dev = platform_get_drvdata(pdev);
...
static int __init stm_miphy_dummy_style_init(void)
{
	int result;
...
postcore_initcall(stm_miphy_dummy_style_init);
