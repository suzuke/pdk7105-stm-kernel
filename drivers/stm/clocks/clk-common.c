...
static int clk_fs432c65_get_rate(unsigned long input, struct stm_fs *fs,
		unsigned long *rate);
static int clk_fs660c32_dig_get_rate(unsigned long input, struct stm_fs *fs,
			   unsigned long *rate);
/*
 * PLL800
 */
...
static int clk_fs216c65_get_params(unsigned long input, unsigned long output,
			    struct stm_fs *fs)
{
	...
	for (; (ns < 4) && ns_search_loop; ns -= 2, ns_search_loop--)
		for (si = 7; (si >= 0) && deviation; si--) {
			...
			for (m = -16, stop = 0; !stop && deviation; m++) {
				...
				if (p > 32768LL)
					break; /* Already too high. Let's move to next sdiv */
				...
				for (; p2 < 32768ll && (p2 < (p + 2)); p2++) {
					...
					clk_fs216c65_get_rate(input, &fs_tmp, &new_freq);
...
static int clk_fs432c65_get_params(unsigned long input, unsigned long output,
			    struct stm_fs *fs)
{
	...
	for (; (ns < 4) && ns_search_loop; ns -= 2, ns_search_loop--)
		for (si = 7; (si >= 0) && deviation; si--) {
			...
			for (m = -16, stop = 0; !stop && deviation; m++) {
				...
				p = P15 * 32 * nd * input * FS432_SCALING_FACTOR;
				p = div64_u64(p, sd * ns * output * FS432_SCALING_FACTOR);
				p2 = P15 * (uint64_t)(m + 33);
				if (p2 < p) {
					continue; /* p must be >= 0 */ }
				p = p2 - p;
				...
				fs_tmp.sdiv = si;
				fs_tmp.nsdiv = (ns == 1) ? 1 : 0;
				if (clk_fs432c65_get_rate(input, &fs_tmp, &new_freq) != 0)
					continue;
...
static int clk_fs660c32_vco_get_params(unsigned long input, unsigned long output,
			     struct stm_fs *fs)
{
/* Formula
				...
				struct stm_fs *fs)
{
	...
	if (deviation) {
		...
		for (; p2 < 32768ll && (p2 <= (fs->pe + 2)); p2++) {
			...
			if (clk_fs660c32_dig_get_rate(input, &fs_tmp, &new_freq) != 0)
				continue;
			if (new_freq < output)
				new_deviation = output - new_freq;
			else
...
static int clk_fs660c32_dig_get_rate(unsigned long input, struct stm_fs *fs,
			   unsigned long *rate)
{
	unsigned long s = (1 << fs->sdiv); /* sdiv value = 1 << sdiv_reg_value */
	unsigned long ns;  /* nsdiv value (1 or 3) */
	uint64_t res;
